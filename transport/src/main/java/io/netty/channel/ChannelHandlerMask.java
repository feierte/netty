...
final class ChannelHandlerMask {
    ...
    // inbound 事件的掩码，用来判断 handler 是否可以处理 inbound 事件
    static final int MASK_ONLY_INBOUND =  MASK_CHANNEL_REGISTERED |
            MASK_CHANNEL_UNREGISTERED | MASK_CHANNEL_ACTIVE | MASK_CHANNEL_INACTIVE | MASK_CHANNEL_READ |
            MASK_CHANNEL_READ_COMPLETE | MASK_USER_EVENT_TRIGGERED | MASK_CHANNEL_WRITABILITY_CHANGED;
    private static final int MASK_ALL_INBOUND = MASK_EXCEPTION_CAUGHT | MASK_ONLY_INBOUND;

    // outbound 事件的掩码，用来判断 handler 是否可以处理 outbound 事件
    ...
    private static int mask0(Class<? extends ChannelHandler> handlerType) {
        ...
        try {
            if (ChannelInboundHandler.class.isAssignableFrom(handlerType)) {
                // 如果是处理 inbound 事件的 handler，那么具有处理任何 inbound 事件的能力
                mask |= MASK_ALL_INBOUND;

                // 如果该 handler 的 channelRegistered 方法上面有 @io.netty.channel.ChannelHandlerMask.Skip 注解，
                // 则不具备处理 channelRegistered 事件的能力，所以去掉 MASK_CHANNEL_REGISTERED 掩码
